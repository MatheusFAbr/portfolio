        ...
        <div className="grid grid-cols-3 gap-3 sm:grid-cols-4 lg:grid-cols-5 xl:gap-4">
          {technologies.map((tech, index) => (
            <Reveal key={tech.id} delay={Math.min(index, 10) * 0.03}>
              <div
                className="group flex aspect-square flex-col items-center justify-center gap-3 rounded-2xl border border-line bg-surface p-3 text-center transition-[border-color,transform] duration-300 ease-[var(--ease-out-soft)] hover:-translate-y-1 hover:border-line-strong"
              ...
              >
                <svg
                  ...
                  viewBox="0 0 24 24"
                  className="h-7 w-7 fill-ink-faint transition-colors duration-300 group-hover:fill-[var(--brand)] sm:h-8 sm:w-8"
                >
                  <path d={tech.path} />
                </svg>
